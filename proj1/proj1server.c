	...
	while (1) {
		...
		if (client_fd < 0) die("Could not establish new connection");
			...
			if (fork() == 0) {
				...
				ubuf[n] = '\0';
				sprintf(outbuf, "Server: Hello %s, you are visitor number %d to this server\n", ubuf, visits);
				// Send back to client
				err = send(client_fd, outbuf, strlen(outbuf), 0);
				if (err < 0) die("Client write failed");
				
				// Insert name here
				n = recv(client_fd, ubuf, UBUF_SIZE, 0);
				if (n < 0) die("Client read failed");
				ubuf[n] = '\0';
				...
				while (1) 
				{
					num_guesses++;
					uint32_t guess_net;
					if (recv(client_fd, &guess_net, sizeof(guess_net), 0) <= 0) break;
					int guess = ntohl(guess_net);

					if (guess == fixedNum) {
						sprintf(outbuf, "Server/%s: Correct in %i guesses\n", ubuf, num_guesses);
						err = send(client_fd, outbuf, strlen(outbuf), 0);
						if (err < 0) die("Client write failed");
						break;
					} else if (guess < fixedNum) {
						sprintf(outbuf, "Server/%s: Low\n", ubuf);
						err = send(client_fd, outbuf, strlen(outbuf), 0);
						if (err < 0) die("Client write failed");
					} else {
						sprintf(outbuf, "Server/%s: High\n", ubuf);
						err = send(client_fd, outbuf, strlen(outbuf), 0);
						if (err < 0) die("Client write failed");
					}	
